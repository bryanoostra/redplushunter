...
% 'IssueOrder(RemoveRat)'
action_schema([
	type(psj2:'IssueOrder'),
	...
	effects([
		condition(true, [
			fact(Agens,	psj2:hasOrdered,	psj2:'RemoveRat')	% the order has been issued
		])
	])
]).
...
action_schema([
	...
	preconditions([
		...
		%	rule(Patiens, owlr:typeOrSubType, psj2:'CanonBall'),% the canonball
			fact(Target, swc:contains, Patiens),				% patiens is in
	...
	effects([
		...
		condition(true, [
			...
			fact(Patiens,	swc:at,				psj2:'Sea')
		]),
		condition(false, [
			fact(Target,  swc:contains, Patiens) % the patiens is no longer in the canon
		])
	])
]).

% 'FinalizeOrder'
% Action semantics: AGENS hasExecuted Patiens
%action_schema([
%	type(psj2:'FinalizeOrder'),
%	arguments([agens(Agens), patiens(Patiens)]),
%	duration(1),
%	preconditions([
%		condition(true, [
%			fact(_,		psj2:hasOrdered,	Patiens),		% Order has been issued
%			fact(Agens,	swc:hasRole, 		psj2:'Pirate')	% agens is a pirate
%		]),
%		condition(false, [
%			fact(_,	psj2:hasExecuted,		Patiens)
%		])	
%	]),
%	effects([
%		condition(true, [
%			fact(Agens,	psj2:hasExecuted,	Patiens)
%		])
%	])
%]).


% 'PutIn' (former LoadCanon)
% Action semantics: AGENS puts PATIENS in TARGET
action_schema([
	type(fabula:'PutIn'),
	...
	preconditions([
		condition(true, [
			rule(Target, owlr:typeOrSubType, swc:'Container'),		% there is a container,
			fact(Agens, swc:has, Patiens),							% the pirate has the patiens
		%	rule(Instrument, owlr:typeOrSubType, psj:'CanonBall'),	% the canonball
			fact(Agens, swc:at, Location),							% and is located
			fact(Target, swc:at, Location)							% next to the container 
		]),
		condition(false, [
			fact(Target,	swc:contains,		_)					% the container does not contain something else atm
		])
	]),
	effects([
		condition(true, [
			fact(Target, swc:contains, Patiens)
		]),
		condition(false, [
			fact(Agens, swc:has, Patiens)		% the agens no longer has the patiens
		])
	])
]).
...
% ---- Agens takes Patiens from Target location ----
...
%		]),
	...
	effects([
		...
		% Thing is no longer where it was (location/container) --should be at loc of agens all the time
		condition(false, [
		    fact(Patiens, swc:at, 		Target)
		])
	])
...
% ---- Agens takes Patiens out of Instrument container at Target location ----

action_schema([
	type(fabula:'TakeOut'),
	arguments([	agens(Agens), patiens(Patiens), instrument(Instrument), location(Target) ]),
	duration(1),
	preconditions([
		% Both at same location
		condition(true, [
		    fact(Agens, 		swc:at, 			Target),			% agens
		    fact(Instrument,	swc:at,				Target),			% and container at same location
		    rule(Instrument,	owlr:typeOrSubType, swc:'Container'),	% and the target is a container
		    fact(Instrument,	swc:contains,		Patiens)			% which contains the patiens		
		]),
		% Can't take yourself
		condition(false, [
		    rule(Agens,	owlr:is, Patiens)		  
	...
	%	]),
		condition(false, [
		    fact(Instrument,  swc:contains,	Patiens)
		])%,
		%condition(false, [
		%    fact(Patiens,	swc:containedBy,	_)
		%])
