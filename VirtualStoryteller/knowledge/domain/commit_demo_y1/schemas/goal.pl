...
goal_schema([
	type(red:'BringCake'),
	arguments([agens(Agens), patiens(Patiens), target(red:grandma)]),
	preconditions([
		condition(true, [
...
goal_schema([
    ...
    success_conditions([
        ...
        condition(true, [
        	fact(Agens, red:asked_what_to_do, red:grandma)
%            fabula(Act2, rdf:type, red:'WhatToDo'),
%            fabula(Act2, fabula:agens, Agens),
%            fabula(Act2, fabula:patiens, red:grandma)
        ])       
    ])
]).
...
goal_schema([
	...
	preconditions([
		...
		condition(true, [
			fabula(Act, rdf:type, red:'ExpressAnger'),
			fabula(Act, fabula:agens, Agens),
			fabula(Act, fabula:target, Patiens)
		])
	]), 
    success_conditions([
		% Patiens is poisoned
