...
% Jeroen:
% Let's make this stuff recurrent! :D I.e., incl. (sub^n)ClassOf.
% E.g., WalkThisWay (subClassOf) GroundMove (subClassOf).
% Leads to crash...
%rule(S, 'http://www.owl-ontologies.com/OWLRules.owl#classOrSubClassOf', O) :-
%    %rdf_db:rdf_global_id(S, S_exp),
%    %rdf_db:rdf_global_id(O, O_exp),
%    
%    owl_subclass_of(S, O_med),
%	rule(O_med, 'http://www.owl-ontologies.com/OWLRules.owl#classOrSubClassOf', O).    
...
% JB
% S greater than or equal to V

rule(S, 'http://www.owl-ontologies.com/OWLRules.owl#greaterEqual', V) :-
	var(S), var(V), !,
	S > V.
