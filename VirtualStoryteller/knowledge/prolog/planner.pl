...
:- module(planner, [
    plan/4,                     % (?Character, +GoalInCharacter, +Conditions, -Plan) 	The main method for making a plan, given conditions. 
	adapt_plan/5,               % (?Character, +GoalInCharacter, +Conditions, +OldPlan, -Plan) 	The main method for making a plan, given conditions. 
    ...
    executableInternalElement/2,	% (+ Plan, ?Operator) 				Determines which internal element operators in the plan are now executable.     
...
% In a story world with human-controlled characters, max_plandepth(6) results in too much time used for planning (-Thijs)
%red domain, planning to reach an impossible goal: 
%red max_plandepth(5)=60sec?
%red max_plandepth(4)=15sec?
%red max_plandepth(3) -> wolf doesn't move to get food at all
max_plandepth(4).
...
plan(Character, GoalInCharacter, Goals, Plan) :-
    max_plandepth(MxDepth)
,	max_plancost(MxCost)
,	max_framing_operators(MxFOs)
,   pop:iterative_pop(depth, (MxDepth, MxCost, MxFOs), Character, GoalInCharacter, Goals, Plan) 
.
...
adapt_plan(Character, GoalInCharacter, Goals, _, Plan2) :-
	plan(Character, GoalInCharacter, Goals, Plan2).
...
% executableInternalElement for given plan is true, when Op can be bound to an internal element step that is not dependend on unexecuted steps.	
executableInternalElement(Plan, Op) :-
	planStep(Plan, (Name, Op)),
	internal_element_schema(Op),
    firstStep(Plan, (Name, Op)).
