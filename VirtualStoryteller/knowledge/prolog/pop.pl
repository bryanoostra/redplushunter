...
:- module(pop, [
    iterative_pop/6,        % +iterator type, +max, +character, +goal, +goalPerspective, -Plan
    iterative_improv_pop/6, % +iterator type, +max, +character, +goal, +goalPerspective -Plan
    pop/5,                  % +max, +character, +goal, +goalPerspective, -Plan
    startStep/1,            % ?Step
    finishStep/1            % ?Step
    ]).
...
iterative_pop(Type, Max, Character, GoalInCharacter, Goal, Plan) :-    
    makeMinimalPlan(Goal, MinimalPlan)
, 	!
,	iterator(Type, 0, Max, Character, GoalInCharacter, MinimalPlan, Plan).
...
%    depth: iterate over number of steps in the plan
%    cost: iterate over maximum cost of the plan    
iterative_improv_pop(Type, Max, Character, GoalInCharacter, Goal, Plan) :-
    makeMinimalPlan(Goal, MinimalPlan)
, 	!	
,	improv_iterator(Type, 0, Max, Character, GoalInCharacter, MinimalPlan, Plan).
    
    
% iterator over depth
iterator(depth, Current, (D, C, I), Character, GoalInCharacter, OldPlan, Plan) :-
	findPlan((Current, C, I), Character, GoalInCharacter, OldPlan, Plan)
...
;	(		Current1 is Current + 1 
		...
		,	iterator(depth, Current1, (D, C, I), Character, GoalInCharacter, OldPlan, Plan)
	)
	.
	

% iterator over cost
iterator(cost, Current, (D, C, I), Character, GoalInCharacter, OldPlan, Plan) :-
	findPlan((D, Current, I), Character, GoalInCharacter, OldPlan, Plan)
...
;	(		Current1 is Current + 1
		...
		,	iterator(cost, Current1, (D, C, I), Character, GoalInCharacter, OldPlan, Plan)
	)
.
	
	
% iterate over improvisations as well as depth or cost, set Type to either of these
improv_iterator(Type,  Current, (D, C, I), Character, GoalInCharacter, OldPlan, Plan) :-
	iterator(Type, 0, (D, C, Current), Character, GoalInCharacter, OldPlan, Plan)
...
;
	(		Current1 is Current + 1
		...
		,	improv_iterator(Type, Current1, (D, C, I), Character, GoalInCharacter, OldPlan, Plan)
	)
.
...
findPlan(Max, Character, GoalInCharacter, Plan, NewPlan) :-
...
,	chooseOperator(Max, Character, GoalInCharacter, Plan, Step, Condition, Plan2)
,	resolveThreats(Plan2, Plan3)
,	findPlan(Max, Character, GoalInCharacter, Plan3, NewPlan)
.
...
chooseOperator(Max, _, _, CurrentPlan, (AskStepname, AskStepoperator), Condition, NewPlan) :-
...
chooseOperator(Max, Character, GoalInCharacter, CurrentPlan, (AskStepname, AskStepoperator), Condition, NewPlan) :-
...
,	chooseStep(CurrentPlan, (AskStepname, AskStepoperator), Condition, NewPlan)
	% In-Character (IC):
;	chooseAction(Max, Character, GoalInCharacter, CurrentPlan, (AskStepname, AskStepoperator), Condition, NewPlan)
;	chooseExpectation(Max, Character, CurrentPlan, (AskStepname, AskStepoperator), Condition, NewPlan)
;	chooseInternalElement(Max, Character, GoalInCharacter, CurrentPlan, (AskStepname, AskStepoperator), Condition, NewPlan)
...
chooseAction(Max, Character, GoalInCharacter, (Steps, Orderings, Links, (Depth, Cost, Improvs)), AskStep, Condition, NewPlan) :-
...
,	GoalInCharacter = true				% Cannot choose action if perspective of planner is not in-character
...
% chooseExpectation
% put an expectation schema in the plan that achieves the precondition
chooseInternalElement(Max, Character, GoalInCharacter, (Steps, Orderings, Links, (Depth, Cost, Improvs)), AskStep, Condition, NewPlan) :-
	AskStep = (_, AskStepOperator)
,	schemas:in_character(AskStepOperator)
,	nonvar(Character)					% Cannot choose internal element if character is not given
,	GoalInCharacter = false				% Cannot choose internal element for in character goals
,	counterDepth(Max, MaxDepth)
,	counterCost(Max, MaxCost)
,	Depth1 is Depth + 1
,	Depth1 =< MaxDepth
,	internal_element_schema(NewOperator)
%,	schema_agens(NewOperator, Character) % Set agens of chosen schema to given character (WHY?)
,	effectMember(Condition, NewOperator) % See if chosen schema is applicable to precondition to search for
,	Cost1 is Cost + 1
,	Cost1 =< MaxCost
...
,	ignore((
	...
	,	show_step(NewStep, Txt)
	,	show_condition(Condition, CondTxt)
	,	prolog_debug:debug(operator_selection, '   # choosing NEW cognition: ~w', Txt)
	,	prolog_debug:debug(operator_selection, '     because it achieves: ~w', CondTxt)
	,	prolog_debug:debug(operator_selection, '     that is			: ~w', Condition)
	,	prolog_debug:debug(operator_selection, '     of step            : ~w', TxtAsk)
	))
