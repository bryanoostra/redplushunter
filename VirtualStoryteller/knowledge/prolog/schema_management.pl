...
:- module(schemas,
	...
	,	cognition_schema/1
	,	internal_element_schema/1
	,	thread_schema/1
	,	action_selection_schema/1
	,	goal_selection_schema/1
	...
	,	goal_urgency/2
%JB	
	,	goal_pos_motivations/2
	,	goal_neg_motivations/2
	,	goal_pos_motivation_list/2
	,	goal_neg_motivation_list/2
	,	goal_dramatic_choice/2 	%tag goals as alternatives belonging to one choice
	,	schema_location/2	 	%extra schema parameter for narration
%JB	
...
;	internal_element_schema(S)
.

% Internal element schema: for now only cognitions, can be extended with emotions
internal_element_schema(S) :-
	cognition_schema(S)
.
...
schema_kind(S, cognition)			:-  cognition_schema(S).
schema_kind(S, thread) 				:-  thread_schema(S).
schema_kind(S, action_selection) 	:-	action_selection_schema(S).
schema_kind(S, goal_selection) 		:-	goal_selection_schema(S).
...
% Expansion of expectation schema
user:term_expansion(cognition_schema(S), cognition_schema(T)) :-
	expand_term(schema(S), schema(T)).		
...
% Expansion of goal selection schema	
user:term_expansion(goal_selection_schema(S), goal_selection_schema(T)) :-
	expand_term(schema(S), schema(T)).		
	
% Expansion of conditional goal selection schemas
user:term_expansion((goal_selection_schema(S) :- Body), (goal_selection_schema(T) :- Body)) :-
	expand_term(schema(S), schema(T)).				
...
% JB
% Expand dramatic choice
expand_schema_element(dramatic_choice(T), dramatic_choice(T_exp)) :-
	nonvar(T)
,	rdf_db:rdf_global_id(T, T_exp).

% Expand positive motivation-list
%expand_schema_element(positive_motivations(T), positive_motivations(T_exp)) :-
%	nonvar(T)
%,	expand_schema_conditions(T, T_exp).

% Expand negative motivation-list
%expand_schema_element(negative_motivations(T), negative_motivations(T_exp)) :-
%	nonvar(T)
%,	expand_schema_conditions(T, T_exp).
...
%JB
%tag for identifying a choice
goal_dramatic_choice(S, C) :-
	goal_schema(S)
,	has_schema_element(S, dramatic_choice(C), '')	
.

%JB
%positive motivations associated with a goal (from schema), empty when not supplied
goal_pos_motivations(S, C) :-
	goal_schema(S)
,	has_schema_element(S, positive_motivations(C), '')	
.

%JB
%negative motivations associated with a goal (from schema), empty when not supplied
goal_neg_motivations(S, C) :-
	goal_schema(S)
,	has_schema_element(S, negative_motivations(C), '')	
.

%JB
%levert de positieve motivaties geassocieerd met dit goal
%levert elk item van de lijst afzonderlijk op (komt dan in een vector via PrologKB.getPrologSingleVariableList)
goal_pos_motivation_list(S, M) :-
	goal_pos_motivations(S, C)
,	member(M, C)
.

%JB
%levert de negatieve motivaties geassocieerd met dit goal
goal_neg_motivation_list(S, M) :-
	goal_neg_motivations(S, C)
,	member(M, C)
.

% Action of an action selection rule (default: [])
action_selection(S, A) :-
	action_selection_schema(S)
,	has_schema_element(S, selected_action(A), [])
.

% Goal of a goal selection rule (default: [])
goal_selection(S, G) :-
	goal_selection_schema(S)
,	has_schema_element(S, selected_goal(G), [])
.
...
% JB Get location of schema
schema_location(S, X) :-
    schema_arguments(S, A)
,	memberchk(location(X), A)
.
	
	% JB Goal expansion, so that predicates can call the goal with prefixed URIs.
	user:goal_expansion(schema_location(S, A), schema_location(S, A2)) :-
		nonvar(A),
		expand_schema_arguments([location(A)], [location(A2)]).			
