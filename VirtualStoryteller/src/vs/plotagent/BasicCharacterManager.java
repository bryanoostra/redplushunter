...
public class BasicCharacterManager implements ICharacterManager {
	...
	private class CastInformation {		
		...
		public void setHumanControlled(boolean isHumanControlled) {
			humanControlled = isHumanControlled;
		}
	}
	...
	private Vector<String> m_uncastedHumanControlledCharacters;
	...
	public BasicCharacterManager(IPlotAgent owner) {
		...
		m_uncastedHumanControlledCharacters = new Vector<String>();
	...
	public void castCharacter(AID agent, String characterURI) {
		...
		try {
			...
			if (m_uncastedHumanControlledCharacters.contains(characterURI)) {
				logger.info("Making human-controlled immediately.");
				info.setHumanControlled(true);
				m_uncastedHumanControlledCharacters.remove(characterURI);
			}
...
/*			
			if (BasicPlotAgent.ALL_HUMAN_CHARACTERS) {
				GuiEvent ev = new GuiEvent( null, BasicPlotAgent.TAKE_CONTROL);			
				ev.addParameter(characterURI);
				((GuiAgent)ownerAgent.getAgent()).postGuiEvent(ev);
			}
*/		} catch(CastException ce) {
	...
	public boolean isHumanControlled(AID character) {
		CastInformation ci = m_characterInfo.get(character);
		if (ci == null) {
			return false;
		}
		return ci.isHumanControlled();
	}
	
	public boolean isHumanControlled(String characterURI) {
		
		return isHumanControlled(getAgentForStoryWorldRepresentation(characterURI));
	}

	
	public void makeHumanControlled(String characterURI, boolean humanControlled) {
		AID assignedAgent = getAgentForStoryWorldRepresentation(characterURI);
		if (assignedAgent != null) {
			logger.info("Making human controlled: " + humanControlled);
			makeHumanControlled(assignedAgent, humanControlled);
		} else {
			// Remember and make human controlled as soon as an agent is assigned.
			if (humanControlled) {
				logger.info("Waiting to make human controlled till the agent has been casted.");
				m_uncastedHumanControlledCharacters.add(characterURI);
			}
		}
	}
	
	public void makeHumanControlled(AID character, boolean humanControlled) {
		if (character == null) {
			return;
		}
		...
		CastInformation ci = m_characterInfo.get(character);
		if (ci == null) {
			logger.info("No cast information for given character: cannot change control status.");
			return;
		}
		ci.setHumanControlled(humanControlled);
	}	
