...
/**
 * Behaviour for asking a Character Agent to select the next action to perform
 * 
 * @author swartjes
 */
public class InitiateRequestSelectActionBehaviour extends
	...
	public InitiateRequestSelectActionBehaviour(Agent a, AID character) {
		...
		m_characterURI = ((BasicPlotAgent) myAgent).getCharacterManager().getStoryWorldRepresentationForAgent(character);
	...
	public void onStart() {
		...
		((RationalAgent) myAgent).fireEvent(new TurnEvent(myAgent, m_characterURI, TurnEvent.TurnStatus.started));
	}
	
	public int onEnd() {
		
		((RationalAgent) myAgent).fireEvent(new TurnEvent(myAgent, m_characterURI, TurnEvent.TurnStatus.ended));
