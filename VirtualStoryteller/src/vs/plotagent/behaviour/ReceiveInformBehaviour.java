...
/*			Vector<String> chars = ((IPlotAgent) myAgent).getThreadManager ().getCurrentThread ().getCharacters ();
			...
			logger.fine("FabulaElement is a: " + fe.getType());
			((IPlotAgent) myAgent).getFabulaBuilder().addFabulaElement(fe);
			
			// Narrate
			// TODO: wrong place. Eigenlijk wil je FabulaBuilder subclassen naar een NarratingFabulaBuilder, en daar narrate() aanroepen. 
			((BasicPlotAgent) myAgent).narrate(fe);
			return true;
	...
	private void handleOperatorResult(OperatorResult or) {
		...
		logger.info("Handling OperatorResult from World Agent: " + or.getOperator() + " is " + or.getStatus());
		...
		if (or.getStatus() instanceof Finished) {
			...
			// Narrate
			//((BasicPlotAgent) myAgent).narrate(or.getOperator());
		}
		
		// Pass on to perception manager
		((BasicPlotAgent)myAgent).getPerceptionManager().registerOperatorResult(or);

		// Send perceptions and settings to characters
		((BasicPlotAgent)myAgent).getPerceptionManager().informCharacters();
		
		((BasicPlotAgent)myAgent).fireEvent(new OperatorResultEvent(this, or));
		...
		if (or.getStatus() instanceof Finished || or.getStatus() instanceof Aborted) {
			for (AID character : ((IPlotAgent) myAgent).getCharacterManager().getCastedCharacters()) {
				...
				myAgent.addBehaviour(new SendInformBehaviour(myAgent, receiver, or));
			}
		}		
	}
}
