...
public class MindDisplayer extends UpdateablePanel {
	...
	class InternalElementNode extends DefaultMutableTreeNode {
		public InternalElementNode(Object name) {
			super(name);
		}
	}
	...
	class MindTreeRenderer extends DefaultTreeCellRenderer {
		...
		ImageIcon ieIcon	= new ImageIcon("img/internal_element.gif");
		...
		public Component getTreeCellRendererComponent(
        	...
        	if (value instanceof InternalElementNode) {
        		setIcon(ieIcon);
        	}
	...
	protected JTree createMindTree() {
		...
		for (Iterator<Operator> it = ExecutionState.getInstance().performingOperatorIterator(); it.hasNext(); ) {
			...
			} else if (currOperator instanceof InferenceOperator) {
				curr = new InternalElementNode(pm.shortForm(currOperator.getType()));
			} else {
