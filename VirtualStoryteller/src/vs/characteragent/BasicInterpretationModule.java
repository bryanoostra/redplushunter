...
public class BasicInterpretationModule implements IInterpretationModule {
	...
	public Set<StoryBelief> adopt(StorySettingElement sse) {
		...
		// Add setting element to fabula 
		// TODO strange, why is it added here and not in the PA?
		// - Added to the PA too.
		// - Does CA ever need to know explicit SE fabula?  
		// - Before removal, make sure there are no timing issues (i.e. further appraisal/coping depending on setting elements in the fabula)
		_fabulaCollector.addFabulaElement(sse);
	...
	public Set<StoryBelief> interpret(StoryPerception sp) {
		...
		// Add perception to fabula 
		// TODO PA also logs perception, is duplicate necessary for episodic memory?
		_fabulaCollector.addFabulaElement(sp);  
	...
	public Set<StoryBelief> reinterpret(StoryBelief sb) {		
		...
		// TODO: trigger rules or something, to determine new beliefs following from sb.
	...
	private void processBeliefs(Set<StoryBelief> sbSet, FabulaElement cause) {
		for (StoryBelief sb: sbSet) {
			...
			// First, store the belief in the fabula			
			...
			// Then, adopt it as truth
			assertBelief(sb);
		}
	}
	
	private void assertBelief(StoryBelief sb) {
		// Process content triples
		...
		// Process content fabula.
		Iterator fI = sb.getAllContentFabula();
		
		while (fI.hasNext()) {
			Object o = fI.next();
			if (o instanceof FabulaElement) {
				// Save contents of beliefs as private facts (should not end up in fabula)
				m_ownerAgent.getEpisodicMemory().addFabulaElement((FabulaElement)o);
				
			} else if (o instanceof FabulaCausality) {
				// Save contents of beliefs as private facts (should not end up in fabula)
				m_ownerAgent.getEpisodicMemory().addFabulaCausality((FabulaCausality)o);
			}
		}
	}	
