		...
		try {
			...
			if (actionContent instanceof SelectAction) {
				...
				if (storyAction == null) {
					...
					logger.info("Sending story action: " + storyAction.getType() + " (" + storyAction.getIndividual() + ")");
			...
			if (actionContent instanceof FramingOperatorPossible) {
				...
				logger.warning("Blindly AGREEing to execution of framing operator. Instead, I should check whether it is consistent with my world view.");
				reply.setPerformative(ACLMessage.AGREE);
			}
