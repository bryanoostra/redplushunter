...
public class BasicOperatorScheduler implements IOperatorScheduler { 
	...
	private static final boolean USE_BODY_CONFLICT = false;
	
	/** set to true, the WA will randomly make some actions a little longer, to ensure story variation */
	private static final boolean RANDOMIZE_DURATIONS = false;
	...
	public int calcDuration(Operator theOperator) {
		int duration = PrologKB.getInstance().getOperatorDuration(theOperator.getPrologDescription());
		if (RANDOMIZE_DURATIONS) {
			if (Math.random() > 0.8) {
				// 20% chance on extra duration
				duration += 1;
			}
		}
		return duration;
	...
	private void narrate(Operator operator) {
		...
		// TODO: this also removes quotes WITHIN narration...instead we should 'trim' quotes not remove em.
		narration = PrologKB.removeQuotes(narration); 
		((BasicWorldAgent)m_ownerAgent).narrate(narration);
	}

	private void performOperator(Operator operator) {
		logger.info("Trying to perform operator " + operator.getPrologDescription());
	...
	public boolean schedule(Operator newOperator) {
		...
		if (!(matchPreconditions(newOperator))) {
			...
			// Informing Plot Agent of failure is also done by abort(newScheduledOperator)
			/*
			OperatorResult or = new OperatorResult();
			...
			*/
			...
			for (Object element : m_operatorSchedule) {
				...
				if ((so.getOperator().getAgens() != null) 
					...
					if (introducesBodyConflict(newOperator, so.getOperator())) {
						...
						// Problem: this may introduce loops where similar actions keep overriding each other
						//abort(so);
						
						// Default: abort the NEW action. This way, actions scheduled will always be fully executed.
						
						// Current: Randomize, for variation
						ScheduledOperator toAbort;
						Vector<ScheduledOperator> choices = new Vector<ScheduledOperator>();
						choices.add(so);
						choices.add(newScheduledOperator);
						
						logger.fine("Choosing random operator to abort from this list: " + choices);
						toAbort = Chooser.randomChoice(choices);
						...
						if (logger.isLoggable(jade.util.Logger.INFO))
							logger.log(jade.util.Logger.INFO, "(t="
									...
									+ toAbort.getOperator().getIndividual()
									+ " was aborted because there is a body conflict with another action ");
						//return false;  // if we want to REFUSE the performance of the new operator
					}
			...
			// REFUSE if we have just aborted the new operator
			if (newScheduledOperator.getStatus() == ScheduledOperator.ABORTED) {
				return false;
			}
	...
	private boolean introducesBodyConflict(Operator op1, Operator op2 ) {
		return USE_BODY_CONFLICT && PrologKB.getInstance().isResourceConflict(op1.getPrologDescription(), op2.getPrologDescription());
