...
public class FabulaPanel extends UpdateablePanel implements GraphMouseListener {
	...
	public static boolean USE_LONG_CAUSAL_NAMES = false;
	public static String PHI = USE_LONG_CAUSAL_NAMES ? "phi" 		: "\u03C6";
	public static String PSI = USE_LONG_CAUSAL_NAMES ? "psi" 		: "\u03C8";
	public static String MOT = USE_LONG_CAUSAL_NAMES ? "motivates" 	: "m";
	public static String ENA = USE_LONG_CAUSAL_NAMES ? "enables" 	: "e";
	...
	public FabulaPanel(IFabulaBuilder builder, StoryDomain sd) {
		...
		logger.info("Reading ontology: " + _storyDomain.getOntology());
        ...
        JButton saveEPS = new JButton("Save EPS");
            ...
            public void actionPerformed(ActionEvent e) {
                ...
                if (returnVal == JFileChooser.APPROVE_OPTION) {
                    try {
                        ...
                        epsWriter = new PrintWriter(new FileWriter(chooser.getSelectedFile()), true);
	...
	public String getVertexInfo(FabulaVertex fv) {
		...
		if (content != null && !fv.getRDFType().equals("DramaticChoice")) { //JB lelijke uitzondering voor dramatic choice
	...
	protected Graph makeGraph(Graph baseGraph) {
		...
		for (StmtIterator it = _reasoningModel.listStatements(); it.hasNext(); ) { 
			...
			if (s.getPredicate().equals(_reasoningModel.getProperty(Fabula.motivates))) {
				...
				g.addEdge(FabulaEdge.getEdge(MOT, src, tgt), src, tgt);
			}	
			if (s.getPredicate().equals(_reasoningModel.getProperty(Fabula.enables))) {
				...
				g.addEdge(FabulaEdge.getEdge(ENA, src, tgt), src, tgt);
			}
		...
		public Paint transform(Object o) {
			...
			if (m_pi.isPicked(o)) {
				...
				if (o instanceof FabulaEdge) {
					...
					if (e.getName().equals(ENA)) {
						return new Color(196,196,196); 
					} 
					if (e.getName().equals("resolves")) {
						return new Color(196,196,255);
					} 
				}					
				return Color.black;
			}
    ...
    public static void main(String args[]) {
    	...
    	final JFrame interfaceFrame = new JFrame("Fabula viewer");
    	...
    	if (returnVal == JFileChooser.APPROVE_OPTION) {
        	...
        	BasicFabulaBuilder builder = new BasicFabulaBuilder(null);
        	NamedGraphFabulaKnowledgeBase kb = new NamedGraphFabulaKnowledgeBase();
        	kb.readFabula(file, "TRIG");
        	builder.registerFabulaKnowledgeBase(kb);
        	FabulaPanel fp = new FabulaPanel(builder, RationalAgent.readStoryDomain());
        	fp.update();
        	interfaceFrame.add(fp);
