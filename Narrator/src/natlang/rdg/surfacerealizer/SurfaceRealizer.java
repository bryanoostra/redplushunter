...
{	
	...
	public SurfaceRealizer(OntModel model){
		reg = new ReferringExpressionGenerator(model);
	...
	public boolean transform() throws Exception
	{
		...
		if (Narrator.TRANSFORMRELS)
		{
			RelationTransformer relTrans = new RelationTransformer(graph);
		...
		{						//expressions and generate the surface form
			...
			{				
				...
				OrderedDepTreeModel order = new OrderedDepTreeModel(tree);
