...
import natlang.rdg.lexicalchooser.CharacterModel;
...
public class ReferringExpressionGenerator implements RDGTransformer,
	 ...
	 *
	 */
	public ReferringExpressionGenerator(OntModel model) {
		history = new EntityHistory();
		characters = CharacterInfo.getCharacterInfoObject();
		pa = new PossibleAdjectives();
		...
		prevsu = "";
		prevobj = "";
		if (Narrator.BRIDGING)
			im = new InferenceModule(model, history, characters, ha);
	}
				...
				if (target.getData().get("pos") != null
					...
					if (ch != null) {
						...
						if (pron) {
							...
							String pronoun = getPronoun(lb, gender, det);
					...
					{
						Entry e = lc.getEntry(target.getData().get("root"),
								true);
	...
	public RSDepTreeNode createNpNode(String label, String entity, Vector adj) {
		...
		if (Narrator.BRIDGING && useMoreGeneralConcept(entity, concept)) {
		...
		if (re != null && history.getEntity(re.getEnt2()) != null) {
			...
			if (highestSalience(re.getEnt2(), "") && Narrator.REFEXP) {
		...
		// example: bedroom01 -> de slaapkamer van de prinses (or haar slaapkamer)
			...
			if (Narrator.BRIDGING) //) && history.getEntity(bte.getEnt2()) != null)
			...
			// use a van-clause
			if (highestSalience(bte.getEnt2(), "") && Narrator.REFEXP) {
		...
		// example gate01 -> de poort van het kasteel, of de poort
			...
			int bridge = InferenceModule.VANINDEF;
			if (Narrator.BRIDGING) // && history.getEntity(poe.getEnt2()) != null)
...
//			if (bridge > 1) {
				if (highestSalience(poe.getEnt2(), "") && Narrator.REFEXP) {
...
//			}
			...
			int bridge = InferenceModule.VANINDEF;
			if (Narrator.BRIDGING) // && history.getEntity(ce.getEnt2()) != null)
...
//			if (bridge > 1) {
				if (highestSalience(ce.getEnt2(), "") && Narrator.REFEXP) {
		...
		if (Narrator.ADDADJECTIVES) {
			...
			if (adjs.size() == 0) {
				...
				if (!a.equals("")) {
					...
					a = pa.getPossibleAdjective(concept);
					if (!a.equals("")) {
