...
public class OrderedDepTreeModel extends RSDepTreeModel implements LibraryConstants
{
	 ...
	 */
	public OrderedDepTreeModel(RSDepTreeModel tree, Lexicon lexicon)
	{			
		super(tree.getRoot());	
		...
		{			
			...
			if (comp instanceof MLabeledEdge)
			{
	...
	private void orderTree(RSDepTreeNode source, RuleHistory history) throws Exception
	{
		...
		while (it.hasNext())
		{
			...
			if (o instanceof MLabeledEdge){
				System.out.println(source);
				//MLabeledEdge m  = (MLabeledEdge) o;
				//System.out.println(m.getSource());;
			}
			if (o instanceof MOrderedEdge )
				l.add(o);
		}
	...
	private void orderWithRule(RSDepTreeNode source, Rule r) throws Exception
	{
		...
		if (!r.applies(edges))	//check whether the selected rule applies
		{
			for (int i=0; i<edges.size(); i++)
			{
				...
				if (edge instanceof MOrderedEdge){
					MOrderedEdge e = (MOrderedEdge) edge; 
		...
		for (int i=0; i < edges.size(); i++)
		{
			...
			if (edge instanceof MOrderedEdge){
				...
				int num = r.getPosition(e.getLabel());	//get the position of this edge
				e.setPosition(num);					//and communicate it to the edge		
			}
	...
	private StringBuffer getSurfaceForm(RSDepTreeNode node) throws Exception	
	{
		...
		while (true)
		{
			...
			{
				Morphology morph = new Morphology(lexicon);
	...
	private RSDepTreeNode getNode(RSDepTreeNode source, int i)
	{
		...
		while (it.hasNext())
		{
			Object o = it.next();
			if (o instanceof MOrderedEdge){
				MOrderedEdge edge = (MOrderedEdge) o;
