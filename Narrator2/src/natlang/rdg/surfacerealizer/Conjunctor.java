...
public class Conjunctor implements RDGTransformer, LibraryConstants
{
	 ...
	 */
	public Conjunctor(RhetRelation rel, RSDepTreeNode node, CharacterInfo characters, LexicalChooser lc)
	{
		cueWord = node;
		relation = rel;
		chars = characters;
	...
	private void changeTense() throws Exception
	{
		...
		{	// if the sentence is in passive voice there already is a vc node, so in that
			...
			else
			{
				...
				if (perfect)
				{
					String auxverb = verb.getData().getEntry().getAuxverb();
					data.set(RSTreeNodeData.ROOT, lc.getEntry(auxverb, false).getRoot());
					data.setEntry(lc.getEntry(auxverb, false));
				...
				else
				{
					...
					data.setEntry(lc.getEntry("will", false));
				}
