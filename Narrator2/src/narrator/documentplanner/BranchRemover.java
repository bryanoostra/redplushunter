 ...
 *
 */
public class BranchRemover implements GraphTransformer
{
	RSGraph graph;
...
//				transformDepthFirst((RSVertex) it.next());
	...
	{			
		...
		{			
			...
			//To fix a bug where the program crashes when the story has only one element.
			if (!pe.getParent().hasNext()) return;
